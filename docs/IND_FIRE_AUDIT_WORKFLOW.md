...
- **Source of truth:** `scripts/nodes/ind_*.js`, written into the JSON by
  ...
  regenerated — see §6.
- **Tests:** `node scripts/test_india.mjs` — 265 assertions, no n8n or database
...
| Test coverage | 265 offline assertions | 110 offline assertions |
...
- `008_record_signoff_row_lock.sql` — **a correctness fix to 007; apply it.**
  ...
  re-read `CONFIRMED`, and be refused. See
  [SIGNOFF_DESIGN.md](SIGNOFF_DESIGN.md) §16.1.
- `009_org_scoping.sql` — adds `org_id` to both audit tables and to
  `field_audit_signoffs`, the tenant scope that organisation-scoped roles require.

  Before it, the Records query filtered on `site_id` alone — and `site_id` is
  caller-supplied — so with two customers a user could read another organisation's
  audits by guessing one, and two customers naming a building `SITE-001` would share
  rows outright. See [SIGNOFF_DESIGN.md](SIGNOFF_DESIGN.md) §17.

  **Nullable on purpose.** Nothing can populate it yet: `/api/audit` has no session to
  derive an organisation from. Existing rows are backfilled `ORG-KRATU-INTERNAL`,
  visibly internal. `VALIDATE_Input` ignores any `org_id` in the request body — a
  caller-chosen tenant scope is not a scope. Add `NOT NULL` only after auth ships.

**Function definitions now live in `scripts/db/functions/`.** `record_signoff()` had
three near-identical copies across three migrations, which made "what does it
currently do?" depend on knowing which migration was newest — a bad question about
the only sanctioned write path to a compliance table.
`scripts/db/functions/record_signoff.sql` is canonical and re-applied whenever it
changes; the copies in 007 and 008 are marked superseded. **Install order: migrations
001–009 in order, then everything in `functions/`.**
...
`scripts/test_india.mjs`, 265 assertions, no n8n, no database, no model call. It
...
| Offline tests | ✅ 265 assertions, restricted sandbox (7.7) |
...
notifier no longer loses messages to an unescaped ampersand, and 265 assertions run

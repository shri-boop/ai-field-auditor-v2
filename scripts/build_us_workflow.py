...
def build():
    ...
    nodes.append({
        "parameters": {
            ...
            # Header Auth, matching audit-history. Every audit is a paid vision
            # call, so an unauthenticated webhook is a metered spend endpoint that
            # anyone who learns the URL can drain.
            #
            # No credentials block is emitted, and that is intentional: the
            # credential ID is generated by n8n, so it cannot be committed here.
            # Bind an httpHeaderAuth credential with header x-audit-api-key in the
            # n8n UI. Until it is bound the webhook rejects every request, which is
            # the correct default for this endpoint — the same way the history
            # workflow ships.
